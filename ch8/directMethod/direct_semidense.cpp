...
// 返回：true为成功，false失败
void poseEstimationDirect ( const vector<Measurement>& measurements, cv::Mat* gray, Eigen::Matrix3f& intrinsics, Eigen::Isometry3d& Tcw );
    ...
    for ( int index=0; index<10; index++ )
    {
        ...
        if ( index ==0 )
        {
            ...
            for ( int x=10; x<gray.cols-10; x++ )
                for ( int y=10; y<gray.rows-10; y++ )//剔除靠近边缘的点
                {
                    Eigen::Vector2d delta (
                        gray.ptr<uchar>(y)[x+1] - gray.ptr<uchar>(y)[x-1], 
                        gray.ptr<uchar>(y+1)[x] - gray.ptr<uchar>(y-1)[x]
                    );//(ΔI(u),ΔI(v))
                    if ( delta.norm() < 50 )//计算二范数
                        continue;
        ...
        {
            if ( rand() > RAND_MAX/5 )//随机抽样显示部分点的匹配情况
                continue;
